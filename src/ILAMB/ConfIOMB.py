...
def VariableReduce(var, region="global", time=None, depth=None, lat=None, lon=None):
    ...
    if lat is not None:
        ...
        wgt = (
            earth_rad
            * (np.sin(var.lat_bnds[:, 1]) - np.sin(var.lat_bnds[:, 0]))[lat0:latf]
        )
        np.seterr(over="ignore", under="ignore")
        out.data = np.ma.average(
            out.data[..., lat0:latf, :], axis=-2, weights=wgt / wgt.sum()
        )
    ...
    if lon is not None:
        ...
        np.seterr(over="ignore", under="ignore")
        out.data = np.ma.average(
            out.data[..., lon0:lonf], axis=-1, weights=wgt / wgt.sum()
        )
        np.seterr(over="raise", under="raise")
        out.lon = None
        out.lon_bnd = None
...
def TimeLatBias(ref, com):
    ...
    area = np.diff(db)[:, np.newaxis] * (
        earth_rad * (np.sin(lb[:, 1]) - np.sin(lb[:, 0]))
    )
    return Variable(
        name=ref.name.replace("timelonint", "timelonbias"),
        unit=ref.unit,
        data=data,
        area=area,
        lat=l,
...
def CycleBias(ref, com):
    ...
    return Variable(
        name=ref.name.replace("cycle", "cyclebias"),
        unit=ref.unit,
        data=data,
        time=mid_months,
...
class ConfIOMB(Confrontation):
    def __init__(self, **keywords):
        ...
        # Get/modify depths
        self.depths = np.asarray(
            self.keywords.get("depths", [0, 100, 250]), dtype=float
        )
        with Dataset(self.source) as dset:
            ...
            if len(depth_name) == 0:
                ...
                data = (
                    dset.variables[dset.variables[depth_name].bounds][...]
                    if "bounds" in dset.variables[depth_name].ncattrs()
                    else dset.variables[depth_name][...]
                )
                self.depths = self.depths[
                    (self.depths >= data.min()) * (self.depths <= data.max())
                ]
        ...
        with Dataset(self.source) as dset:
            for attr in dset.ncattrs():
                pages[-1].text += "<p><b>&nbsp;&nbsp;%s:&nbsp;</b>%s</p>\n" % (
                    attr,
                    str(dset.getncattr(attr)).encode("ascii", "ignore"),
                )
    ...
    def stageData(self, m):
        ...
        with Dataset(self.source) as dset:
            ...
            if climatology:
                info += "[climatology]"
                obs_cb = (obs_cb - 1850) * 365.0
                t0 = obs_cb[0]
                tf = obs_cb[1]
            else:
                t0 = obs_tb[0, 0]
                tf = obs_tb[-1, 1]
            info += " contents span years %.1f to %.1f, est memory %d [Mb]" % (
                t0 / 365.0 + 1850,
                tf / 365.0 + 1850,
                obs_mem,
            )
        ...
        if len(vname) == 0:
            logger.debug(
                "[%s] Could not find [%s] in the model results"
                % (self.name, ",".join(possible))
            )
        ...
        for fname in m.variables[vname]:
            with Dataset(fname) as dset:
                ...
                mod_t, mod_tb, mod_cb, mod_b, mod_e, cal = il.GetTime(
                    var, t0=t0 - m.shift, tf=tf - m.shift
                )
                if mod_t is None:
                ...
                mod_mem += mem
                info += (
                    "\n      %s spans years %.1f to %.1f, est memory in time bounds %d [Mb]"
                    % (
                        fname,
                        mod_t.min() / 365.0 + 1850,
                        mod_t.max() / 365.0 + 1850,
                        mem,
                    )
                )
        info += "\n      total est memory = %d [Mb]" % mod_mem
        logger.info(
            "[%s][%s][%s] reading model data from possibly many files%s"
            % (self.name, m.name, vname, info)
        )
        if mod_t0 > mod_tf:
            logger.debug(
                "[%s] Could not find [%s] in the model results in the given time frame, tinput = [%.1f,%.1f]"
                % (self.name, ",".join(possible), t0, tf)
            )
            raise il.VarNotInModel()
        ...
        if climatology:
            ...
            logger.info(
                "[%s][%s] building climatology in %d slabs" % (self.name, m.name, ns)
            )

            # across what times?
            slab_t = (mod_tf - mod_t0) * np.linspace(0, 1, ns + 1) + mod_t0
            slab_t = (
                np.floor(slab_t / 365) * 365
                + bnd_months[
                    (np.abs(bnd_months[:, np.newaxis] - (slab_t % 365))).argmin(axis=0)
                ]
            )
            ...
            for i in range(ns):
                ...
                mind = (np.abs(mid_months[:, np.newaxis] - (v.time % 365))).argmin(
                    axis=0
                )
                if data is None:
    ...
    def confront(self, m):
        def _addDepth(v):
            v.depth = np.asarray([5.0])
            v.depth_bnds = np.asarray([[0.0, 10.0]])
        ...
        with il.FileContextManager(self.master, mod_file, obs_file) as fcm:
            ...
            for obs, mod in self.stageData(m):
                ...
                for depth in self.depths:
                    ...
                    z = mod
                    if mod.layered:
                        z = z.integrateInDepth(z0=depth - 1.0, zf=depth + 1, mean=True)
                    z = z.integrateInTime(mean=True)
                    mod_timeint[dlbl].append(
                        Variable(
                            name="timeint%s" % dlbl,
                            unit=z.unit,
                            data=z.data.reshape((1,) + z.data.shape),
                ...
                for region in self.regions:
                    z = VariableReduce(
                        obs, region=region, time=tb[0], lon=[-180.0, +180.0]
                    )
                    z.time = t
                    z.time_bnds = tb
                    z.temporal = True
                    z.data.shape = (1,) + z.data.shape
                    obs_depth[region].append(z)

                    z = VariableReduce(
                        mod, region=region, time=tb[0], lon=[-180.0, +180.0]
                    )
                    z.time = t
                    z.time_bnds = tb
                    z.temporal = True
                ...
                for region in self.regions:
                    ...
                    i = (np.abs(mid_months[:, np.newaxis] - (z.time % 365))).argmin(
                        axis=0
                    )
            ...
            for dlbl in obs_timeint.keys():
                ...
                mod_tmp = il.CombineVariables(mod_timeint[dlbl]).integrateInTime(
                    mean=True
                )
                ...
                if dlbl == "0":
                    with np.errstate(all="ignore"):
                        bias_score = Variable(
                            name=bias.name.replace("bias", "biasscore"),
                            data=np.exp(-np.abs(bias.data) / large_bias),
                            unit="1",
                            ndata=bias.ndata,
                            lat=bias.lat,
                            lat_bnds=bias.lat_bnds,
                            lon=bias.lon,
                            lon_bnds=bias.lon_bnds,
                            area=bias.area,
                        )
            ...
            for region in self.regions:
                mod_tmp = il.CombineVariables(mod_depth[region]).integrateInTime(
                    mean=True
                )
                mod_tmp.name = "timelonint_of_%s_over_%s" % (self.variable, region)
                mod_tmp.toNetCDF4(fcm.mod_dset, group="MeanState")
                obs_tmp = il.CombineVariables(obs_depth[region]).integrateInTime(
                    mean=True
                )
                ...
                np.seterr(over="raise", under="raise")
                mcyc[region] = Variable(
                    name="cycle_of_%s_over_%s" % (self.variable, region),
                    unit=mod.unit,
                    data=mcyc[region],
                    depth=mod.depth,
    ...
    def modelPlots(self, m):
        ...
        # model plots
        cmap = {"timeint": self.cmap, "bias": "seismic", "biasscore": "score"}
        plbl = {"timeint": "MEAN", "bias": "BIAS", "biasscore": "BIAS SCORE"}
        with Dataset(fname) as dataset:
            ...
            for ptype in ["timeint", "bias", "biasscore"]:
                for vname in [v for v in variables if ptype in v]:
                    var = Variable(
                        filename=fname, variable_name=vname, groupname="MeanState"
                    )
                    try:
                    ...
                    for region in self.regions:
                        ...
                        fig.savefig(
                            "%s/%s_%s_%s.png"
                            % (self.output_path, m.name, region, vname)
                        )
                        plt.close()
        ...
        for region in self.regions:
            ...
            if vname in variables:
                var0 = Variable(
                    filename=bname, variable_name=vname, groupname="MeanState"
                )
                var = Variable(
                    filename=fname, variable_name=vname, groupname="MeanState"
                )
                bias = Variable(
                    filename=fname,
                    variable_name=vname.replace("timelonint", "timelonbias"),
                    groupname="MeanState",
                )
                if region == "global":
                    page.addFigure(
                    ...
                    page.addFigure(
                        "Overlapping mean regional depth profiles",
                        "timelonints",
                ...
                fig.savefig(
                    "%s/%s_%s_timelonint.png" % (self.output_path, m.name, region)
                )
                ax.set_ylim((min(d0.max(), d.max()), max(d0.min(), d.min())))
                fig.savefig(
                    "%s/%s_%s_timelonints.png" % (self.output_path, m.name, region)
                )
                plt.close()
                fig, ax = plt.subplots(figsize=(6.8, 2.8), tight_layout=True)
                ...
                ax.set_ylim((min(d0.max(), d.max()), max(d0.min(), d.min())))
                fig.savefig(
                    "%s/%s_%s_timelonbias.png" % (self.output_path, m.name, region)
                )
                plt.close()
            ...
            if vname in variables:
                ...
                bias = Variable(
                    filename=fname,
                    variable_name=vname.replace("cycle", "cyclebias"),
                    groupname="MeanState",
                )
                if region == "global":
                    page.addFigure(
                        "Mean regional annual cycle",
                        "cycle",
                    ...
                    page.addFigure(
                        "Overlapping mean regional annual cycle",
                        "cycles",
                ...
                ax.set_ylim((min(d0.max(), d.max()), max(d0.min(), d.min())))
                fig.savefig(
                    "%s/%s_%s_cyclebias.png" % (self.output_path, m.name, region)
                )
                plt.close()
            ...
            for ptype in ["timeint"]:
                for vname in [v for v in variables if ptype in v]:
                    ...
                    for region in self.regions:
                        ...
                        fig = ax.get_figure()
                        fig.savefig(
                            "%s/Benchmark_%s_%s.png" % (self.output_path, region, vname)
                        )
                        plt.close()
        ...
        for region in self.regions:
            ...
            if vname in variables:
                ...
                ax.set_ylabel("depth [m]")
                fig.savefig(
                    "%s/Benchmark_%s_timelonint.png" % (self.output_path, region)
                )
                ax.set_ylim((min(d0.max(), d.max()), max(d0.min(), d.min())))
                fig.savefig(
                    "%s/Benchmark_%s_timelonints.png" % (self.output_path, region)
                )
                plt.close()
            ...
            if vname in variables:
                ...
                ax.set_ylabel("depth [m]")
                fig.savefig(
                    "%s/%s_%s_cycle.png" % (self.output_path, "Benchmark", region)
                )
                ax.set_ylim((min(d0.max(), d.max()), max(d0.min(), d.min())))
                fig.savefig(
                    "%s/%s_%s_cycles.png" % (self.output_path, "Benchmark", region)
                )
                plt.close()
        ...
        for fname in glob.glob("%s/*.nc" % self.output_path):
            with Dataset(fname) as dataset:
                ...
                variables = [
                    ...
                    if (
                        v not in group.dimensions.keys()
                        and "_bnds" not in v
                ...
                for vname in variables:
                    ...
                    if "_over_" in vname:
                        ...
                        limits[pname]["min"] = min(
                            limits[pname]["min"], var.getncattr(min_str)
                        )
                        limits[pname]["max"] = max(
                            limits[pname]["max"], var.getncattr(max_str)
                        )
        ...
        for pname in limits.keys():
            if "score" in pname:
                if "min" in limits[pname].keys():
                    limits[pname]["min"] = 0.0
                    limits[pname]["max"] = 1.0
                else:
                    for region in limits[pname].keys():
                        limits[pname][region]["min"] = 0.0
                        limits[pname][region]["max"] = 1.0
        self.limits = limits
        ...
        for pname in limits.keys():
            ...
            if base_pname in ["bias", "timelonbias", "cyclebias"]:
                if "min" in limits[pname]:
                    ...
                    vabs = max(
                        abs(limits[pname]["global"]["max"]),
                        abs(limits[pname]["global"]["min"]),
                    )
