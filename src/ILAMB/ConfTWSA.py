...
class ConfTWSA(Confrontation):
    ...
    def __init__(self, **keywords):
        ...
        basin_source = self.keywords.get(
            "basin_source",
            Path(os.environ["ILAMB_ROOT"]) / "DATA/mrro/Dai/basins_0.5x0.5.nc",
        )
    ...
    def stageData(self, m):
        ...
        obs.data = odata
        obs.ndata = odata.shape[1]
        obs.spatial = False
        mod.data = mdata
        mod.ndata = mdata.shape[1]
        mod.spatial = False
    ...
    def confront(self, m):
        ...
        iav_score = Variable(
            ...
            unit="1",
            data=np.exp(
                -np.abs(mod_anom_val.data - obs_anom_val.data) / obs_anom_val.data
            ),
        )
