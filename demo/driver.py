...
ta, co2amp, co2ampstd = il.ComputeDecadalAmplitude(dt,co2)
ta,dco2amp,dco2ampstd = il.ComputeDecadalAmplitude(dt,dco2)
pt = ta/365.+1850
...
ax[1,1].text(xmax-0.05*dx,ymin+0.05*dy,
             "Bias = %+.2f (%.2f)\nRMSE = %.2f (%.2f)" % (il.ComputeNormalizedBias(dco2amp,co2amp),
                                                            il.ComputeNormalizedBias(dco2ampstd,co2ampstd),
                                                            il.ComputeNormalizedRootMeanSquaredError(dco2amp,co2amp),
                                                            il.ComputeNormalizedRootMeanSquaredError(dco2ampstd,co2ampstd)),
             ha="right",va="bottom")
